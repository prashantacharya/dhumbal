...
import { validateDrop, canDeclare, calculateRoundScores, type Card } from './gameLogic.js';
...
const playerRooms = new Map<string, string>(); // Map socket.id -> roomId

type CreateRoomPayload = { playerName: string };
type JoinRoomPayload = { roomId: string; playerName: string };
type SetReadyPayload = { ready: boolean };
type DropCardsPayload = { cardIds: string[] };
type PickCardPayload = { fromDeck: boolean; cardId?: string };
...
io.on('connection', (socket: Socket) => {
  ...
  // Create room
  socket.on('create-room', ({ playerName }: CreateRoomPayload) => {
    try {
  ...
  // Set player ready
  socket.on('set-ready', ({ ready }: SetReadyPayload) => {
    try {
      const roomId = playerRooms.get(socket.id);
      if (!roomId) {
  ...
  // Drop cards
  socket.on('drop-cards', ({ cardIds }: DropCardsPayload) => {
    try {
      const roomId = playerRooms.get(socket.id);
      if (!roomId) {
        socket.emit('error', { message: 'Not in a room' });
      ...
      const cardsToDrop: Card[] = player.hand.filter(c => cardIds.includes(c.id));
  ...
  // Pick card
  socket.on('pick-card', ({ fromDeck, cardId }: PickCardPayload) => {
    try {
      const roomId = playerRooms.get(socket.id);
      if (!roomId) {
        socket.emit('error', { message: 'Not in a room' });
