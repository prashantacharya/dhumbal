...
export class GameRoom {
  ...
  currentPlayerDroppedCards: Card[];
  ...
  roundHistory: Array<{
    roundNumber: number;
    scores: Array<{ playerId: string; playerName: string; handValue: number; score: number }>;
  ...
  addPlayer(playerId: string, playerName: string): { success: boolean; reason?: string } {
  ...
  dropCards(playerId: string, cardIds: string[]): DropResult {
  ...
  pickCard(playerId: string, fromDeck: boolean = true, cardIdToPick: string | null = null): PickResult {
    if (this.currentTurn !== playerId) {
      return { success: false, reason: 'Not your turn' };
    }
  ...
  declare(playerId: string): DeclareResult {
    if (this.currentTurn !== playerId) {
      return { success: false, reason: 'Not your turn' };
    }
  ...
  endRound(roundScores: Array<{ playerId: string; playerName: string; handValue: number; score: number }>): EndRoundResult {
  ...
  getPublicState(): {
    roomId: string;
    players: Array<{ id: string; name: string; handCount: number; totalScore: number; isReady: boolean }>;
    gameState: GamePhase;
    currentTurn: string | null;
    turnStartTime: number | null;
  ...
  getPlayerState(playerId: string): (ReturnType<GameRoom['getPublicState']> & { hand: Card[]; isCurrentTurn: boolean }) | null {
    const player = this.players.find(p => p.id === playerId);
    if (!player) {
      return null;
    }
...
export class RoomManager {
  ...
  createRoom(roomId: string, hostId: string, hostName: string): { success: boolean; reason?: string; room?: GameRoom } {
